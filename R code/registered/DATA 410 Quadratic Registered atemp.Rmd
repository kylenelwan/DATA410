---
title: "DATA 410 Quadratic Registered atemp"
author: "Kyle Nelwan"
date: "04/04/2022"
...
## QUADRATIC SETUP AND BOX-COX TRANSFORMATION
...
The quadratic setup is done by squaring all the explanatory variables, and see if the eventual transformed fit is better than the linear/un-squared explanatory variables transformed fit.

After the Box-Cox transformation, the optimal lambda is 0.222. Visually, from the graph, the lambda = 0 value seems to be not within the 95% CI. Therefore, the actual optimum lamda will be used without approximation, consequently, the dependent variable will be (y^lambda - 1)/lambda (instead of log(y) if the lambda = 0).
...
X <- subset(x, select = - c(casual,registered,weekday2,weekday))
X <- as.matrix(X)
summary(lm(y~X))
plot(lm(y~X))
```
